...
class ProximityContentManager: NearestBeaconManagerDelegate {
    ...
    func nearestBeaconManager(nearestBeaconManager: NearestBeaconManager, didUpdateNearestBeaconID nearestBeaconID: BeaconID?) {
        if let nearestBeaconID = nearestBeaconID {
            self.beaconContentFactory.contentForBeaconID(beaconID: nearestBeaconID) { (proximityContent) in
                self.delegate?.proximityContentManager(proximityContentManager: self, didUpdateContent: proximityContent)
            }
        } else {
            self.delegate?.proximityContentManager(proximityContentManager: self, didUpdateContent: nil)
        }
    }
